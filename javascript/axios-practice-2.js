...
const getProductsReviews = async (num) => {
  const res = await axios.get(`${BASE_URL}/products/${num}`);
  console.log(res["data"]["reviews"]);
};

//getProductsReviews(10);
...
const getProductsReviewsRating = async (num) => {
  const res = await axios.get(`${BASE_URL}/products/${num}`);
  let sum = 0;
...
const getProductsSearch = async (str) => {
  const res = await axios.get(`${BASE_URL}/products/search?q=${str}`);

  console.log(res["data"]["total"]);
};

getProductsSearch("glass");
/* 출력 결과
// phone 검색 시
23
